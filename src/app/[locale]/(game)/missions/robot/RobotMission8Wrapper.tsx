...
const RobotMission8 = dynamic(
  () => import('@/components/missions/robot/RobotMission8'),
...
export default function RobotMission8Wrapper({ initialScore = 0 }: { initialScore?: number }) {
  ...
  const hints = [
    tMissions('robot.m8.hint1'),
    tMissions('robot.m8.hint2'),
  ]
  ...
  const handleComplete = useCallback(async (
    ...
    const throughputScore = scoreBreakdown.find(b => b.max === 500)?.value ?? 0
    const qualityScore = scoreBreakdown.find(b => b.max === 350)?.value ?? 0

    const slotsCorrect = throughputScore >= 500 ? 5 : throughputScore >= 400 ? 4 : throughputScore >= 250 ? 3 : 1
  ...
  return (
    <MissionShell
      missionTitle={tMissions('robot.m8.missionTitle')}
      missionNumber={8}
      ...
      skillsPracticed={locale === 'ru' ? ['Оптимизация', 'Системное мышление', 'Контроль качества'] : locale === 'ar' ? ['التحسين', 'التفكير المنهجي', 'مراقبة الجودة'] : ['Optimization', 'Systems Thinking', 'Quality Control']}
      xpEarned={xpEarned}
    ...
    >
      <RobotMission8 onComplete={handleComplete} />
    </MissionShell>
  )
}
