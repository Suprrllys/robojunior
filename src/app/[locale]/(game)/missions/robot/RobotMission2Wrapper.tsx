...
const RobotMission2 = dynamic(
  () => import('@/components/missions/robot/RobotMission2'),
...
export default function RobotMission2Wrapper({ initialScore = 0 }: { initialScore?: number }) {
  ...
  const hints = [
    tMissions('robot.m2.hint1'),
    tMissions('robot.m2.hint2'),
  ]
  ...
  const handleComplete = useCallback(async (
    ...
    const choiceScore = scoreBreakdown.find(b => b.max === 500)?.value ?? 0
    const testingScore = scoreBreakdown.find(b => b.max === 350)?.value ?? 0

    // Map choice score to slots: Legs=5/5, Tracks=3/5, Wheels=1/5
    const slotsCorrect = choiceScore >= 500 ? 5 : choiceScore >= 300 ? 3 : 1
    // Map testing thoroughness to tests passed
    const testsPassed = testingScore >= 350 ? 3 : testingScore >= 200 ? 2 : 1
    ...
    try {
      const result = await completeMissionV2({
        ...
        moneySpent: 0,
        budgetLimit: 50,
        testQuality: testingScore >= 350 ? 'smooth' : testingScore >= 200 ? 'jerky' : 'fail',
      ...
      console.error('Failed to save robot mission 2:', err)
  ...
  const handleNext = useCallback(() => {
    router.push('/missions/robot?mission=3')
  }, [router])

  return (
    <MissionShell
      missionTitle={tMissions('robot.m2.missionTitle')}
      missionNumber={2}
      ...
      skillsPracticed={locale === 'ru' ? ['Инженерия', 'Анализ', 'Принятие решений'] : locale === 'ar' ? ['الحركة', 'تحليل التضاريس', 'اتخاذ القرارات'] : ['Engineering', 'Analysis', 'Decision Making']}
      xpEarned={xpEarned}
    ...
    >
      <RobotMission2 onComplete={handleComplete} />
    </MissionShell>
  )
}
