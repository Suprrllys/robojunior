...
const EntrepreneurMission10 = dynamic(
  () => import('@/components/missions/entrepreneur/EntrepreneurMission10'),
...
export default function EntrepreneurMission10Wrapper({ userId, initialScore = 0 }: Props) {
  ...
  const hints = [
    tMissions('entrepreneur.m10.hint1'),
    tMissions('entrepreneur.m10.hint2'),
  ]
  ...
  return (
    <MissionShell
      missionTitle={tMissions('entrepreneur.m10.title')}
      missionNumber={10}
      ...
      skillsPracticed={locale === 'ru' ? ['Построение экосистемы', 'Портфельная стратегия', 'Менторство'] : locale === 'ar' ? ['بناء المنظومة', 'إدارة المحافظ', 'الشراكات'] : ['Ecosystem Building', 'Portfolio Strategy', 'Mentorship']}
      xpEarned={xpEarned}
    ...
    >
      <EntrepreneurMission10 userId={userId} onComplete={handleComplete} />
    </MissionShell>
  )
}
