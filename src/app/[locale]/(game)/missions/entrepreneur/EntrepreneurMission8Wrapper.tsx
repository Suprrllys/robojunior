...
const EntrepreneurMission8 = dynamic(
  () => import('@/components/missions/entrepreneur/EntrepreneurMission8'),
...
export default function EntrepreneurMission8Wrapper({ userId, initialScore = 0 }: Props) {
  ...
  const hints = [
    tMissions('entrepreneur.m8.hint1'),
    tMissions('entrepreneur.m8.hint2'),
  ]
  ...
  const handleComplete = useCallback(async (
    ...
    try {
      ...
      console.error('Failed to save entrepreneur mission 8:', err)
  ...
  const handleNext = useCallback(() => {
    router.push('/missions/entrepreneur?mission=9')
  }, [router])

  return (
    <MissionShell
      missionTitle={tMissions('entrepreneur.m8.title')}
      missionNumber={8}
      ...
      skillsPracticed={locale === 'ru' ? ['Анализ данных', 'Бизнес-аналитика', 'Распределение бюджета'] : locale === 'ar' ? ['تحليل البيانات', 'التسويق', 'استراتيجية النمو'] : ['Data Analysis', 'Business Intelligence', 'Budget Allocation']}
      xpEarned={xpEarned}
    ...
    >
      <EntrepreneurMission8 userId={userId} onComplete={handleComplete} />
    </MissionShell>
  )
}
