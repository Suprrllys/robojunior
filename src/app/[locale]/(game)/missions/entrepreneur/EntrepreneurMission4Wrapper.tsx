...
const EntrepreneurMission4 = dynamic(
  () => import('@/components/missions/entrepreneur/EntrepreneurMission4'),
...
export default function EntrepreneurMission4Wrapper({ userId, initialScore = 0 }: Props) {
  ...
  const hints = [
    tMissions('entrepreneur.m4.hint1'),
    tMissions('entrepreneur.m4.hint2'),
  ]
  ...
  const handleComplete = useCallback(async (
    ...
    try {
      ...
      console.error('Failed to save entrepreneur mission 4:', err)
  ...
  const handleNext = useCallback(() => {
    router.push('/missions/entrepreneur?mission=5')
  }, [router])

  return (
    <MissionShell
      missionTitle={tMissions('entrepreneur.m4.title')}
      missionNumber={4}
      ...
      skillsPracticed={locale === 'ru' ? ['Презентация', 'Сторителлинг', 'Финансовая грамотность'] : locale === 'ar' ? ['جمع التمويل', 'العروض التقديمية', 'التحليل المالي'] : ['Pitching', 'Storytelling', 'Financial Literacy']}
      xpEarned={xpEarned}
    ...
    >
      <EntrepreneurMission4 userId={userId} onComplete={handleComplete} />
    </MissionShell>
  )
}
