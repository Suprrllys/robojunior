...
const EntrepreneurMission9 = dynamic(
  () => import('@/components/missions/entrepreneur/EntrepreneurMission9'),
...
export default function EntrepreneurMission9Wrapper({ userId, initialScore = 0 }: Props) {
  ...
  const hints = [
    tMissions('entrepreneur.m9.hint1'),
    tMissions('entrepreneur.m9.hint2'),
  ]
  ...
  const handleComplete = useCallback(async (
    ...
    try {
      ...
      console.error('Failed to save entrepreneur mission 9:', err)
  ...
  const handleNext = useCallback(() => {
    router.push('/missions/entrepreneur?mission=10')
  }, [router])

  return (
    <MissionShell
      missionTitle={tMissions('entrepreneur.m9.title')}
      missionNumber={9}
      ...
      skillsPracticed={locale === 'ru' ? ['Финансовый анализ', 'Оценка стоимости', 'Переговоры'] : locale === 'ar' ? ['التحليل المالي', 'التفاوض', 'التخطيط الاستراتيجي'] : ['Financial Analysis', 'Valuation', 'Negotiation']}
      xpEarned={xpEarned}
    ...
    >
      <EntrepreneurMission9 userId={userId} onComplete={handleComplete} />
    </MissionShell>
  )
}
