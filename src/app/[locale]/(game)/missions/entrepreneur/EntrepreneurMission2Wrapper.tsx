...
const EntrepreneurMission2 = dynamic(
  () => import('@/components/missions/entrepreneur/EntrepreneurMission2'),
...
export default function EntrepreneurMission2Wrapper({ userId, initialScore = 0 }: Props) {
  ...
  const hints = [
    tMissions('entrepreneur.m2.hint1'),
    tMissions('entrepreneur.m2.hint2'),
  ]
  ...
  return (
    <MissionShell
      missionTitle={tMissions('entrepreneur.m2.title')}
      missionNumber={2}
      ...
      skillsPracticed={locale === 'ru' ? ['Построение команды', 'Управление бюджетом'] : locale === 'ar' ? ['بناء الفريق', 'التوظيف', 'إدارة الميزانية'] : ['Team Building', 'Budget Management']}
      xpEarned={xpEarned}
    ...
    >
      <EntrepreneurMission2 userId={userId} onComplete={handleComplete} />
    </MissionShell>
  )
}
